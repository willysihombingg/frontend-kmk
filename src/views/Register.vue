<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 px-4">
    <div class="max-w-md w-full">
      <h2 class="text-center text-2xl font-bold text-gray-800 mb-6">Sign Out</h2>
      ...
      <div class="mt-2 text-sm text-gray-600">
        Already Account? <a href="/login" class="font-medium text-indigo-600 hover:text-indigo-500">Back To Login</a>
      </div>
    </div>
  </div>
</template>
...
<script>
import { register } from '../networking/api-request'
export default {
  data() {
    return {
      name: '',
  ...
  methods: {
    async onRegister() {
      const result = await register(this.name, this.nim, this.department, this.handphone, this.email, this.password)
      console.log(result)
      if (result.data) {
