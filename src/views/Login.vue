...
import { login } from '../networking/api-request'
export default {
  data() {
    return {
      nim: '',
  ...
  async mounted() {
    ...
    //   const response = await axios.post('http://localhost:3030/api/v1/users/login', )
  ...
  methods: {
    async onLogin() {
      const result = await login(this.nim, this.password)
      console.log(result)
      if (result.data) {
        localStorage.setItem('token', result.data.data.token)
        this.$router.push('/')
      } else if (result.message == 'Request failed with status code 422') {
